...
Переход на `hnswlib` (C++) дал радикальное ускорение построения: весь диапазон m=4–12 укладывается в 0.9–1.6 с. Recall растёт с m уверенно — от 0.631 при m=4 до 0.844 при m=12, при этом размер индекса меняется незначительно (153–161 MB). QPS снижается с ростом m (7 331 → 3 576), так как плотнее связанный граф требует проверки большего числа рёбер.
